...
//¿Por qué si no reservo memoria me da un segmentation fault?
...
void * cuentaLineas (void * argv){
    ...
    int *numLineas=malloc(sizeof(int));     //Reservamos memoria para un int
...
int main(int argc, const char * argv[]){
    ...
    if (argc < 2){
        printf("Error debe llamar al programa de la siguiente forma: ./ej2 fichero1.txt, fichero2.txt, ..., ficheron.txt)");
        exit(EXIT_FAILURE);
    }
    ...
    for (int i=0; i<numArchivos; i++){
        ...
        if (pthread_create(&(thread[i]), NULL, (void *) cuentaLineas, (void*) argv[i+1])){      //Si la creacion de la hebra nos devuelve algo distinto a 0, habra ocurrido un error
